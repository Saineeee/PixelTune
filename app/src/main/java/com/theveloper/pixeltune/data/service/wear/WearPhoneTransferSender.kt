...
import com.theveloper.pixeltune.shared.WearCapabilities
...
class WearPhoneTransferSender @Inject constructor(
    ...
    suspend fun isPixelTuneWatchAvailable(): Boolean {
        return runCatching {
            val capability = capabilityClient.getCapability(
                WearCapabilities.PixelTune_WEAR_APP,
                CapabilityClient.FILTER_REACHABLE,
            ).await()
            capability.nodes.isNotEmpty()
        }.getOrElse { error ->
            Timber.tag(TAG).w(error, "Failed checking PixelTune Wear availability")
            false
        }
    }
    ...
    suspend fun requestSongTransfer(songId: String): Result<Int> {
        return runCatching {
            ...
            if (nodes.isEmpty()) {
                error("No reachable watch with PixelTune")
            }
