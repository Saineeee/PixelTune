...
@Database(
    ...
    version = 25, // Incremented for the file_path lookup index

    exportSchema = false
)
abstract class PixelTuneDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * Add the index backing `MusicDao.getSongByPath()` (file-path lookups,
         * e.g. M3U import resolution) so it no longer full-scans `songs`.
         *
         * `parent_directory_path` is re-asserted with IF NOT EXISTS purely as
         * a no-op hardening measure for installs whose index set diverged from
         * the entity; on healthy v24 databases the statement does nothing.
         * Index names follow Room's `index_<table>_<columns>` convention so the
         * post-migration schema validation matches the entities exactly.
         */
