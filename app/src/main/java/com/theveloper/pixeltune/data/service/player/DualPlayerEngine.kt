...
class DualPlayerEngine @Inject constructor(
    ...
    suspend fun resolveCloudUri(uri: Uri): Uri {
        ...
        val resolved: Uri? = when (uri.scheme) {
            ...
            "soundcloud" -> resolveSoundCloudUriAsync(uri)
            else -> null
        }
    ...
    private suspend fun resolveYouTubeUriAsync(uri: Uri): Uri? {
        val youtubeId = uri.host ?: return null
        Timber.tag("DualPlayerEngine").d("Async resolving YouTube URI: $youtubeId")
        ...
        val proxyUrl = youtubeStreamProxy.getProxyUrl(youtubeId)
        return if (proxyUrl.isNotEmpty()) Uri.parse(proxyUrl) else null
    }

    private suspend fun resolveSoundCloudUriAsync(uri: Uri): Uri? {
        val encodedUrl = uri.host ?: return null
        Timber.tag("DualPlayerEngine").d("Async resolving SoundCloud URI: $encodedUrl")
        ...
        val proxyUrl = soundcloudStreamProxy.getProxyUrl(encodedUrl)
        return if (proxyUrl.isNotEmpty()) Uri.parse(proxyUrl) else null
    }
