...
class WidgetUpdateReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != "com.example.PixelTune.ACTION_WIDGET_UPDATE_PLAYBACK_STATE") return
        ...
        scope.launch {
            try {
                ...
                val glanceIds = glanceAppWidgetManager.getGlanceIds(PixelTuneGlanceWidget::class.java)
                glanceIds.forEach { glanceId ->
                    PixelTuneGlanceWidget().update(context, glanceId)
                }
