...
import com.theveloper.pixeltune.data.media.ImageCacheManager
import com.theveloper.pixeltune.data.media.MetadataEditError
...
import com.theveloper.pixeltune.utils.FileDeletionUtils
    ...
    suspend fun saveMetadata(
        ...
        val finalCoverArtUpdate = if (coverArtUpdate == null) {
            val existingMetadata = try {
                 com.theveloper.pixeltune.data.media.AudioMetadataReader.read(java.io.File(song.path))
            } catch (e: Exception) {
                null
            }
